...
def create_boxplots(simple_cosines, compound_cosines, complex_cosines):
    ...
    plt.savefig("openai_large_sentence_types_boxplot.png", format="png")
...
# keep english, spanish, german, hindi, chinese languages in translations

translations = translations[['sentence', 'en', 'es', 'de', 'fr', 'cs']]
# translations.drop(columns=['ja'], inplace=True)
translations.set_index('sentence', inplace=True)

with open('openai_embeddings_large.json', 'r') as file:
...
print("Cross-lingual similarity of simple sentences for OpenAI Embeddings", simple_cosine_avg)
print("Cross-lingual similarity of compound sentences for OpenAI embeddings", compound_cosine_avg)
print("Cross-lingual similarity of complex sentences for OpenAI Embeddings", complex_cosine_avg)